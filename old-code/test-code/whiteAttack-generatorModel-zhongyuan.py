...
from net import generator_model
...
def inverse(train_data, test_data, device, args):
    ...
    for i in range(3):
        ...
        localEntry = trn_X[0]  # 本地数据
        targetEntry = trn_X[1]  # 要恢复的数据

        initdata = torch.randn(targetEntry.size()).to(device)
        inputEntry = torch.cat((localEntry, initdata), dim=1)


        GRN = generator_model(input_dim=inputEntry.shape[-1],intern_dim=inputEntry.shape[-1],output_dim=targetEntry.shape[-1]).to(device)
        ...
        for j in range(args.NIters):  # 迭代优化
            ...
            totalLoss = featureLoss  + 0.1*floss + 0.01*bloss + nloss
        ...
        # 第2次优化
        # xGen = xGen.clone().detach().to(device)
        ...
        #     # 现在换用欧几里得距离
        #     euclidean_dist = torch.nn.functional.pairwise_distance(xGen, targetEntry)
        #     wandb.log({"euclidean_dist_xGen_" + str(i): euclidean_dist})
        #     wandb.log({"similarity_xGen_" + str(i): similarity})
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', type=str, default='vfl_whiteAttack_a1', help='experiment name')
