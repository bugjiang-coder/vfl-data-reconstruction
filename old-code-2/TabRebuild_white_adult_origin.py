...
from fedml_core.utils.utils import tensor2df, Similarity, bool_loss, int_loss, neg_loss, normalize_loss
from fedml_core.utils.utils import adult_dataset, over_write_args_from_file, num_loss, tabRebuildAcc
...
def rebuild(train_data, tab, device, args):
    ...
    for i in range(args.Ndata):
        ...
        xGen = torch.randn(originData.size()).to(device)
        ...
        for j in range(args.NIters):  # 迭代优化
            ...
            totalLoss = featureLoss + args.numloss * numloss + args.iloss * iloss + args.bloss * bloss + args.nloss * nloss + args.norloss * norloss
