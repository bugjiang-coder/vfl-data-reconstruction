...
def run_experiment(device, args, ratio=0.5):
    ...
    trainset = IndexedCIFAR10(A_ratio=ratio, root=args.data_dir, train=True, download=True, transform=train_transform)
    testset = IndexedCIFAR10(A_ratio=ratio, root=args.data_dir, train=False, download=True, transform=train_transform)
    ...
    top_model = TopModelForCifar10(A_ratio=ratio)
    ...
    if args.resume:
        if os.path.isfile(args.resume+str(ratio)):
            print("=> loading checkpoint '{}'".format(args.resume+str(ratio)))
            checkpoint = torch.load(args.resume, map_location=device)
            args.start_epoch = checkpoint['epoch']
            vfltrainer.load_model(args.resume+str(ratio), device)
    ...
    vfltrainer.save_model(args.save, 'best.pth.tar' + str(ratio), epoch, top1_acc)
...
if __name__ == '__main__':
    ...
    for r in ratio_list:
        print("radio: ", r)
        run_experiment(device=device, args=args, ratio=r)
