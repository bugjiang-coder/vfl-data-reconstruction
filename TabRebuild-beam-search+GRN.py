...
def rebuild(train_data, models, tab, device, args):
    ...
    for i in range(args.Ndata):
        ...
        initdata = xGenList[0].to(device)
        initdata.requires_grad = True
        ...
        # 构建生成模型
        GRN = generator_model(input_dim=inputEntry.shape[-1], intern_dim=inputEntry.shape[-1],
        ...
        for j in range(int(args.NIters)):  # 迭代优化
            optimizer.zero_grad()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', type=str, default='vfl-beam-search+GRN+3sigma+2beam', help='experiment name')
    ...
    parser.add_argument('--origin_data_output', default="/home/yangjirui/paper-code/data/zhongyuan/norm-2layer/bsGNR/origin_data.csv",
                        help='location of the data corpus')
    parser.add_argument('--inverse_data_output', default="/home/yangjirui/paper-code/data/zhongyuan/norm-2layer/bsGNR/inverse_data.csv",
                        help='location of the data corpus')
    # ==========下面是几个重要的超参数==========
    ...
    parser.add_argument('--granularity', type=int, default=20, help="Beam search granularity")
    parser.add_argument('--beam_size', type=int, default=10, help="Beam search beam size (granularity>=beam_size)")
    parser.add_argument('--norloss', type=float, default=0.00001, help="Recovery data negative number loss intensity")
